...
def isGPGGAinString(inputString):
    if inputString.startswith("$GNGGA"): #replace 1 == 1 with condition to be checked for inputString
        return True
    else:
        return False
...
def rospub():
    pub = rospy.Publisher('gps', Customrtk, queue_size=100)
